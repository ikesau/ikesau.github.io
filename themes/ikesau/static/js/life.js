        ...
        .map(
          (_, xi) =>
            new Cell(
              ...
              undefined,
              createVector(
                100 + random(-MUTATION_FACTOR, MUTATION_FACTOR),
                100,
                100
              )
            )
...
function meanVector(vectors) {
  return vectors
    .reduce((acc, cur) => p5.Vector.add(acc, cur), new p5.Vector())
    .div(vectors.length);
}
class Cell {
  ...
  checkWillSurvive() {
    ...
    if (
      this.isAlive &&
      (aliveNeighbours.length === 3 || aliveNeighbours.length === 2)
    ) {
      return true;
    }
  ...
  breed() {
    ...
    if (!this.isAlive && aliveNeighbours.length === 3) {
      const averageFillColour = meanVector(
        aliveNeighbours.map((cell) => cell.fillColour)
      );
      const mutatedFillColour = averageFillColour.add(
        createVector(
          random(-MUTATION_FACTOR, MUTATION_FACTOR),
          random(-MUTATION_FACTOR, MUTATION_FACTOR)
        )
      );
...
function draw() {
  background(0);
  translate(
    (CELL_SIZE + (windowWidth % CELL_SIZE)) / 2,
    (CELL_SIZE + (windowHeight % CELL_SIZE)) / 2
  );
  scale(CELL_SIZE);
...
function addCell() {
  ...
  const gridY = floor(
    (mouseY / windowHeight) * floor(windowHeight / CELL_SIZE)
  );
  const cell =
    (_a = grid[gridY]) === null || _a === void 0 ? void 0 : _a[gridX];
  if (cell) {
